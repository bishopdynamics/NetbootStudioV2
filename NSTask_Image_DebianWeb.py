#!/usr/bin/env python3
"""
Netboot Studio Task for creating new boot image for Debian webinstaller
"""
...
class NSTask_Image_DebianWeb(NSTask_Image_Builder):
    # create Debian webinstaller boot image
    required_keys = ['name', 'comment', 'debian_release', 'kernel_args', 'create_unattended']  # declare required keys an they will be checked at init
    build_dependencies = []
    ...
    def update_metadata(self):
        ...
        try:
            ...
            self.bootimage_metadata['image_type'] = 'debian-webinstaller'
    ...
    def generate_ipxe(self):
        ...
        imgexec || goto failed
        ''' % (self.task_payload['debian_release'], self.task_payload['kernel_args']))
...
if __name__ == "__main__":
    ...
    try:
        TEST_TASK = {
            'task_type': 'image_debian_webinstaller',
            'task_payload': {
                'create_unattended': True,
                'debian_release': 'bullseye',
                'name': 'Jacks debian webinstaller test build',
                'kernel_args': 'ipv6.disable=1 IPV6_DISABLE=1 net.ifnames=0 biosdevname=0',
                'comment': 'Testing debian-webinstaller',
            },
            'task_id': str(uuid.uuid4()),
            'task_name': 'New Debian Webinstaller boot image',
            'task_description': 'Create a new Debian Web installer boot image'
        }
        PATHS_TEST = build_paths('/opt/NetbootStudio')
        taskobj = NSTask_Image_DebianWeb(PATHS_TEST, send_message, TEST_TASK)
        taskobj.start()
