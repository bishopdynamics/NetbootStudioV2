...
let URL_UPLOADSERVER = 'https://' + window.location.hostname + ':' + UPLOADSERVER_PORT;

// function ShowUploadStatus(file_id, file_name, status, progress, description){
// 	try {
// 		let uploadID = 'upload_' + strip_slashes(file_id);
// 		let ns_message = new NSMessage();
// 		let content = {
// 			task_id: uploadID,
...
// 		};
// 		ns_message.set_key('content', content);
// 		ns_message.set_key('topic', 'task_status');
// 		ns_message.set_key('target', 'All');
// 		ns_message.set_key('sender', 'FileUploader');
// 		status_callback(ns_message);
// 	} catch (ex) {
...
function SetupUploader(target_div_id, file_destination, status_callback){
	...
	let allowed_destinations = ['certs', 'ipxe_builds', 'ipxe_stage1', 'iso', 'packages', 'stage4', 'tftp_root', 'uboot_scripts', 'unattended_configs']
	let uppy = null;
		...
		try {
			uppy = Uppy.Core({
				...
				onBeforeFileAdded: function (currentFile, files) {
					...
					try {
						let files_datasource = getDataSourceByName(this.nsFileDestination)
						if (files_datasource !== null) {
							let files_array = files_datasource.get_value();
			...
			uppy.on('upload-progress', function (file, pdata) {
				let progress = (pdata.bytesUploaded / pdata.bytesTotal * 100).toFixed(2);
